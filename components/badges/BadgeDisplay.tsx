...
function BadgeIcon({ badge, size, userRole }: BadgeIconProps) {
  ...
  return (
    ...
    >
      ...
      {isHovered && (
        <div className="absolute top-full left-1/2 z-50 mt-2 -translate-x-1/2 whitespace-nowrap rounded-lg border border-white/20 bg-black/90 px-3 py-1.5 text-xs font-medium text-white shadow-lg backdrop-blur-sm">
          {badge.name}
          <div className="absolute left-1/2 bottom-full -translate-x-1/2 border-4 border-transparent border-b-white/20" />
        </div>
      )}
    </div>
  )
